...
export const RemixCreatedBox = ({ meme }: RemixShareBoxProps) => {
    ...
    return (
        <div className="flex flex-col space-y-4 comic-border bg-light-green p-10 rounded-4xl text-center w-3/4 h-1/2">
            <h3 className="text-xl font-extrabold uppercase whitespace-pre-line">{"your meme is live!\nshare with your frens."}</h3>
            <button onClick={handleShareBean} className="comic-border rounded-full bg-share-gradient px-20 py-3 text-base">
            ...
            <button onClick={handleShareMemixer} className="comic-border-mini rounded-full bg-white px-20 py-2 text-base font-medium">See on Memixer</button>
            <button onClick={handleShareLenster} className="comic-border-mini rounded-full bg-white px-20 py-2 text-base font-medium">See on Lenster</button>
            <div>
                <p className="font-medium mt-4 mb-1 text-xs">Still felling funny?</p>
                <RemixBtn disabled={false} onClick={handleRemixAgain} btnText="Remix again" className="mx-auto create-btn-gradient rounded-full border-black border-solid border-4 px-3 max-w-max text-lg font-medium " />
            </div>
        </div>
    )
}